...
<Cards>
  <Card title="Organization RBAC" href="/docs/permissions/org-rbac">
    Org-level capabilities: who may create spaces, manage members, and manage roles. Built on Better
    Auth's dynamic access control.
  </Card>
  <Card title="Space & page access" href="/docs/permissions/space-page-access">
    Who may read and write **content** within a specific space or page, via space roles and optional
    per-page ACLs.
  </Card>
</Cards>
...
<Callout title="Always enforced server-side">
  Both layers are enforced on the server. The frontend only **mirrors** them to gate UI — never rely
  on the client for enforcement, because the server always re-checks.
</Callout>
